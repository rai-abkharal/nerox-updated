...
export class VpnService {
  static async getServers(userId: string) {
    ...
    const userRes = await pool.query(
      'SELECT plan_type, subscription_end_date FROM users WHERE user_id = $1',
      [userId]
    );
  ...
  static startMonitoring() {
    ...
    setInterval(async () => {
      try {
        const { rows: servers } = await pool.query('SELECT server_id, hostname, status FROM vpn_servers');
        for (const server of servers) {
          ...
          if (server.status !== newStatus) {
             ...
             await pool.query(
               ...
               [server.server_id, newStatus, isFailing ? 'Automated health check failure' : 'Back online']
             );
          }
          ...
          await pool.query(
            `UPDATE vpn_servers 
             ...
             WHERE server_id = $5`,
            [cpuUsage, latency, loadChange, newStatus, server.server_id]
          );
          ...
          if (!isFailing) {
            await pool.query(
              `INSERT INTO server_metrics (server_id, cpu_usage, avg_latency_ms, active_connections) 
               VALUES ($1, $2, $3, (SELECT current_load FROM vpn_servers WHERE server_id = $1))`,
              [server.server_id, cpuUsage, latency]
            );
          }
        }
      } catch (err) {
  ...
  static async getSmartServers(userId: string, amount: number = 3, preferredProtocol: string = 'Auto') {
    ...
    if (rows.length === 0 && !isPremium) {
      const fallbackRes = await pool.query(
        'SELECT * FROM vpn_servers WHERE status = $1 AND "is_premium" = FALSE LIMIT $2',
        ['active', amount]
      );
  ...
  static async startSession(userId: string, serverId: string, clientIp: string) {
    ...
    const userRes = await pool.query(
      'SELECT plan_type, subscription_end_date, trial_ends_at, preferred_protocol, split_tunneling_config FROM users WHERE user_id = $1',
      [userId]
    );
    ...
    const serverRes = await pool.query(
      'SELECT server_id, hostname, ip_address, "is_premium", protocol, location, country_code FROM vpn_servers WHERE server_id = $1 AND status = $2',
      [serverId, 'active']
    );
    ...
    if (!isPremium && !isTrialActive) {
      ...
      const usageRes = await pool.query('SELECT daily_data_used_bytes FROM users WHERE user_id = $1', [userId]);
    ...
    const { rows } = await pool.query(
      `INSERT INTO vpn_sessions (user_id, server_id, client_ip, assigned_vpn_ip, status) VALUES ($1, $2, $3, $4, 'active') RETURNING session_id, assigned_vpn_ip`,
      [userId, serverId, clientIp, assignedVpnIp]
    );
    
    await pool.query('UPDATE vpn_servers SET current_load = current_load + 1 WHERE server_id = $1', [serverId]);
  ...
  private static encryptConfig(text: string): string {
    const algorithm = 'aes-256-cbc';
    // Ensure the key is exactly 32 bytes by hashing it
    const rawKey = process.env.VPN_CONFIG_KEY || '64_char_hex_key_here_for_prod_env_fallback';
  ...
  private static generateVpnConfig(hostname: string, protocol: string, assignedIp: string): string {
...
ifconfig ${assignedIp} 255.255.255.0
  ...
  static async endSession(sessionId: string) {
    ...
    if (rows.length > 0) {
      ...
      await pool.query(
        'UPDATE vpn_servers SET current_load = GREATEST(0, current_load - 1) WHERE server_id = $1',
        [serverId]
      );
    }
