...
export class PaymentService {
  static async verifyPurchase(userId: string, platform: string, productId: string, purchaseToken: string) {
    ...
    try {
      ...
      const planRes = await client.query(
        `SELECT * FROM subscription_plans WHERE ${platform === 'android' ? 'google_product_id' : 'apple_product_id'} = $1`,
        [productId]
      );
      ...
      const subRes = await client.query(
        ...
        [userId, plan.plan_id, startDate, endDate, platform === 'android' ? 'Google Play' : 'Apple IAP']
      );
      ...
      // Log Transaction
      await client.query(
        `INSERT INTO payment_transactions (subscription_id, amount, status, processed_at, platform, purchase_token)
         VALUES ($1, $2, 'completed', NOW(), $3, $4)`,
        [subscriptionId, plan.price_usd, platform, purchaseToken]
      );
